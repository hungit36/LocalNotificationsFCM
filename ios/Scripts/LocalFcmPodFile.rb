...
def install_local_fcm_ios_pod_target(application_path = nil)
...
def update_local_fcm_service_target(target_name, xcodeproj_path, flutter_root)
     ...
     if target.nil? || project.targets.count == 1
         raise "You need to create a Notification Service Extension to properly use local_push_notifications_fcm\n"
     end
     target.build_configurations.each do |config|
